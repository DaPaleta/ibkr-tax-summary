...
import { extract867Fields, headerRates, FIELDS_867 } from "./extract.js"
...
function hasRateHeader(pages) {
  return pages.some((p) => p.rows.some((r) => headerRates(r).length >= 2))
}
...
export async function parse867(pdfPath) {
  const parser = new PDFParse({ data: new Uint8Array(await readFile(pdfPath)) })
